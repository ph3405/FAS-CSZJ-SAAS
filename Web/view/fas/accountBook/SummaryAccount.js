layui.config({
    ...
    var send = function (request, index) {
        $.Post("/fas/accountBook/SummaryListSearch", request,
        function (data) {
            var res = data;
            if (res.IsSuccess) {
    ...
    function GetSumKMHtml(saveData) {
        if (saveData != undefined && saveData.Data.length > 0) {
            //html = "";
            var htmlStyle =
                "<style> body {text-align: center;} .tbSumKM td {" +
                ...
                ".tbSumKM tbody, .tbSumKM tfoot {" +
                "font-size: 13px;;font-family:'黑体' " +
                "} </style>";
            var count = 0;
            var htmlHead = "";

            var htmlTbl = "<table class='tbSumKM' style='width: 90%;margin: 0 auto;' cellspacing='0' cellpadding='0'>" +
                "<thead>" +
                "<tr style='height: 30px;border:0px'>" +
                "<td colspan='4' style='font-size:20px;text-align:center;font-weight:400;border:0px'>科目汇总表</td>" +
                "</tr>" +
                "<tr style='height: 30px; '>" +
                "<td colspan='4' style='font-size: 12px;border:0px;text-align:center;' ><div style='float:left;width:30%;text-align:left;'>编制单位：" + account + "</div>" +
                nowPeriod +
                "<div style='float:right;width:30%;text-align:right;'>单位：元</div></td >" +
                "</tr > " +

                "<tr>" +
                "<td style='width: 25%; text-align: center'>科目编码</td>" +
                "<td style='width: 25%; text-align: center'>科目名称</td >" +
                "<td style='width: 25%; text-align: center'>借方金额</td>" +
                "<td style='width: 25%; text-align: center'>贷方金额</td>" +
            ...
            for (var i = 0; i < saveData.Data.length; i++) {
                ...
                var Code = saveData.Data[i].Code == "9" ? "" : saveData.Data[i].Code;
                htmlDetail += "<tr>" +
