...
public class FileHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            var itemId = context.Request["itemId"];
            if (file != null)
            {
                var user = SessionHelper.GetUserInfo();

                var dt = ExcelToDataTable("辅助核算", true, file);

                CustomDataBLL bll = new TKS.FAS.BLL.FAS.CustomDataBLL();
                bll.CustomDataImport(dt, user.Token,itemId);
