layui.config({
    ...
    var query = function (pageIndex) {
        ...
        request.PageSize = 10;
        $.Post("/sso/userListSearch", request,
            function (data) {
                var res = data;
                if (res.IsSuccess) {
    ...
    $(".usersAdd_btn").click(function () {
        $.open("添加用户", "/view/ssoUser/userAdd.aspx");
    })
    ...
    $("body").on("click", ".tks-rowRole", function () {  //编辑
        ...
        $.open('设置角色', "/view/ssorole/roleChecked.aspx?id=" + id);

    })

    $("body").on("click", ".tks-rowEdit", function () {  //编辑
        var id = $(this).attr('data-id');
        $.open('用户编辑', "/view/ssouser/userEditor.aspx?id=" + id);
